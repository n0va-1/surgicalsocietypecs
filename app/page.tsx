...
type ContentOrigin = "human" | "ai_assisted" | "ai_generated";
...
type AnnouncementRow = { id?: string; date: string; title: string; hu: string; text: string; target: string; pinned: boolean; publishedAt?: string; contentOrigin?: ContentOrigin };
...
type CurriculumAsset = { id: string; module_id: string; kind: "image" | "video"; object_key: string; caption: string | null; url: string | null; content_origin: ContentOrigin; depicts_identifiable_person: boolean; likeness_consent_confirmed: boolean };
type CurriculumModule = { id: string; level: "beginner" | "intermediate" | "advanced"; week: number; title_en: string; title_hu: string; introduction_en: string | null; introduction_hu: string | null; technique_en: string | null; technique_hu: string | null; application_en: string | null; application_hu: string | null; equipment_en: string | null; equipment_hu: string | null; steps_en: string[]; steps_hu: string[]; video_url: string | null; published: boolean; content_origin: ContentOrigin; editorial_review_confirmed: boolean; reviewed_at: string | null; reviewer_name?: string | null; assets: CurriculumAsset[] };
...
function originDisclosure(origin: ContentOrigin | undefined, language: Language) {
  if (origin === "ai_generated") return language === "hu" ? "MI által létrehozott vagy lényegesen módosított tartalom · ember által ellenőrizve" : "AI-generated or materially AI-altered content · human reviewed";
  if (origin === "ai_assisted") return language === "hu" ? "MI segítségével készült · ember által ellenőrizve" : "AI-assisted content · human reviewed";
  return null;
}
...
export default function Home() {
  ...
  const [announcementOrigin, setAnnouncementOrigin] = useState<ContentOrigin>("human");
  ...
  const [curriculumAssetOrigin, setCurriculumAssetOrigin] = useState<ContentOrigin>("human");
  const [curriculumAssetDepictsPerson, setCurriculumAssetDepictsPerson] = useState(false);
  const [curriculumAssetConsent, setCurriculumAssetConsent] = useState(false);
  ...
  const refreshPortalData = useCallback(async () => {
    ...
    try {
      ...
      if (responses[0].ok && payloads[0].announcements) {
        ...
        return { id:String(item.id), date:date.toLocaleDateString("en-GB",{day:"2-digit",month:"short"}).toUpperCase(), title:String(item.title_en), hu:String(item.title_hu ?? item.title_en), text:String(item.body_en), target:String(item.target_level), pinned:Boolean(item.pinned), publishedAt:String(item.published_at), contentOrigin:String(item.content_origin ?? "human") as ContentOrigin, read:Boolean(item.read) };
        });
  ...
  async function submitPhoto(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const authenticityConfirmed = form.get("authenticityConfirmed") === "on";
    const file = selectedUpload.current;
    if (!file) return;
    if (!authenticityConfirmed) { setToast("Confirm that this is a genuine photograph of your own practice work."); return; }
    ...
    try {
      ...
      const response = await fetch("/api/submissions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({moduleId:module.id,objectKey,reflection,authenticityConfirmed})});
      const result=await response.json();
  ...
  async function publishAnnouncement(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response=await fetch("/api/announcements",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:form.get("title"),message:form.get("message"),target:announcementTarget,pinned:form.get("pinned")==="on",contentOrigin:announcementOrigin,editorialReviewConfirmed:form.get("editorialReviewConfirmed")==="on"})});
    const result=await response.json();
  ...
  if (!authenticated) {
    return <div className="auth-shell" data-theme={theme}>
      ...
      <section className="auth-panel">
        ...
        <div className="auth-card">
          ...
          <form onSubmit={enterPortal}>
            ...
            {authMode === "register" && <label className="consent-row"><input name="privacyAccepted" type="checkbox" required /><span>I have read the <button type="button" onClick={() => setModal("privacy")}>privacy and content-transparency notice</button> and will upload only genuine personal, non-clinical practice images with no patient information.</span></label>}
      ...
      {modal === "privacy" && <ModalShell title={language === "hu" ? "Adatvédelem és tartalmi átláthatóság · jóváhagyandó tervezet" : "Privacy and content transparency · draft for approval"} onClose={() => setModal(null)} wide><PrivacyNotice language={language} /></ModalShell>}
  ...
  function CurriculumPage() {
    ...
    async function saveChapter(event: React.FormEvent<HTMLFormElement>) {
      ...
      const payload = {
        ...
        video_url: form.get("video_url"), content_origin: form.get("content_origin"),
        editorial_review_confirmed: form.get("editorial_review_confirmed") === "on",
        published: isAdmin && form.get("published") === "on",
      };
      setDataBusy(true);
      try {
    ...
    async function uploadAsset(file?: File) {
      ...
      if (curriculumAssetDepictsPerson && !curriculumAssetConsent) { setToast("Confirm consent before uploading media that depicts a recognisable person."); return; }
      ...
      try {
        ...
        if (uploadError) throw uploadError;
        const response = await fetch("/api/curriculum/assets", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ moduleId:selected.id, objectKey, kind:video ? "video" : "image", caption:file.name, contentOrigin:curriculumAssetOrigin, depictsIdentifiablePerson:curriculumAssetDepictsPerson, likenessConsentConfirmed:curriculumAssetConsent }) });
        const result = await response.json();
        if (!response.ok) throw new Error(result.error ?? "The file could not be attached.");
        setToast(`${video ? "Video" : "Image"} added to the chapter with its content origin recorded.`); setCurriculumAssetOrigin("human"); setCurriculumAssetDepictsPerson(false); setCurriculumAssetConsent(false); await refreshPortalData();
    ...
    return <PageFrame eyebrow={isAdmin ? "ADMINISTRATOR · PUBLICATION CONTROL" : "CURRICULUM EDITOR · DRAFT ACCESS"} title="Teaching curriculum" description={isAdmin ? "Review chapters, manage teaching media and decide what students can see." : "Write structured teaching chapters and add images or short videos. Only the administrator can publish them."} action={<button className="primary-button" onClick={() => setSelectedCurriculumId("")}>＋ New chapter</button>}>
      ...
      <div className="curriculum-workspace"><aside className="chapter-list"><div><strong>Chapters</strong><span>{curriculumRecords.length}</span></div>{curriculumRecords.length===0?<p>No chapters yet.</p>:curriculumRecords.map(module=><button type="button" className={selectedCurriculumId===module.id?"active":""} onClick={()=>setSelectedCurriculumId(module.id)} key={module.id}><span>W{module.week}</span><div><strong>{module.title_en}</strong><small>{module.level} · {module.published?"Published":"Draft"}</small></div></button>)}</aside>
        <form className="chapter-editor" key={selected?.id ?? "new"} onSubmit={saveChapter}><div className="chapter-editor-head"><div><span className="eyebrow">{selected ? `WEEK ${selected.week}` : "NEW CHAPTER"}</span><h2>{selected?.title_en ?? "Untitled teaching chapter"}</h2></div>{selected&&<b className={selected.published?"published":"draft"}>{selected.published?"Published":"Draft"}</b>}</div>
          ...
          <section className="provenance-panel"><div><span className="eyebrow">CONTENT ORIGIN</span><h3>How was this chapter prepared?</h3><p>Choose the most accurate option. AI-assisted material still requires complete human medical and editorial review.</p></div><label className="form-field"><span>Chapter text origin</span><select name="content_origin" defaultValue={selected?.content_origin??"human"}><option value="human">Original human-created content</option><option value="ai_assisted">AI-assisted, rewritten and human reviewed</option><option value="ai_generated">AI-generated or materially AI-altered</option></select></label>{isAdmin&&<label className="check-row review-confirmation"><input name="editorial_review_confirmed" type="checkbox" defaultChecked={selected?.editorial_review_confirmed??false} required /><span>I have personally reviewed the complete English and Hungarian chapter for medical accuracy and accept editorial responsibility.</span></label>}{selected?.reviewed_at&&<p className="review-record">Last approved by {selected.reviewer_name??"the administrator"} on {new Date(selected.reviewed_at).toLocaleDateString()}.</p>}</section>
          {selected&&<section className="chapter-assets"><div><strong>Chapter media</strong><label className="secondary-button">＋ Upload image or video<input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm" hidden onChange={event=>{void uploadAsset(event.target.files?.[0]);event.currentTarget.value="";}} /></label></div><div className="asset-origin-controls"><label className="form-field"><span>Origin of the next file</span><select value={curriculumAssetOrigin} onChange={event=>setCurriculumAssetOrigin(event.target.value as ContentOrigin)}><option value="human">Original human-created media</option><option value="ai_assisted">AI-assisted media</option><option value="ai_generated">AI-generated or materially altered</option></select></label><label className="check-row"><input type="checkbox" checked={curriculumAssetDepictsPerson} onChange={event=>{setCurriculumAssetDepictsPerson(event.target.checked);if(!event.target.checked)setCurriculumAssetConsent(false);}} /> It depicts a recognisable real person</label>{curriculumAssetDepictsPerson&&<label className="check-row"><input type="checkbox" checked={curriculumAssetConsent} onChange={event=>setCurriculumAssetConsent(event.target.checked)} required /> I confirm that the person consented to this use and any AI alteration</label>}</div>{selected.assets.length===0?<p>No media uploaded yet. Images may be up to 10 MB; MP4/WebM videos up to 50 MB.</p>:<div className="asset-grid">{selected.assets.map(asset=><article key={asset.id}>{asset.kind==="image"&&asset.url?<Image src={asset.url} width={320} height={200} unoptimized alt={asset.caption??"Curriculum image"}/>:asset.url?<video src={asset.url} controls preload="metadata"/>:<span>Media unavailable</span>}<div>{originDisclosure(asset.content_origin,"en")&&<span className="ai-label">{originDisclosure(asset.content_origin,"en")}</span>}<small>{asset.kind.toUpperCase()}</small><strong>{asset.caption??"Teaching media"}</strong>{asset.depicts_identifiable_person&&<em>Recognisable-person consent recorded</em>}<button type="button" onClick={()=>void removeAsset(asset)}>Remove</button></div></article>)}</div>}</section>}
          <div className="chapter-save">{isAdmin?<label className="check-row"><input name="published" type="checkbox" defaultChecked={selected?.published??false} /> Publish to students after saving</label>:<p>Saved as a private draft. The administrator must complete and record the final medical review before publication.</p>}<button className="primary-button" type="submit" disabled={dataBusy}>{dataBusy?"Saving…":isAdmin?"Save chapter":"Save draft"} →</button></div>
        </form></div>
    </PageFrame>;
  }
  ...
  function AdminPage() {
    async function createStaffCode(event: React.FormEvent) {
      event.preventDefault();
      if (isLocalPreview) { const label=inviteRole==="student"?studentInviteCode:staffInviteEmail; setStaffCodes(current=>[{code:label,status:"Preview",expires:`${inviteRole} · ${inviteMaxUses} use(s)`},...current]); setToast(inviteRole==="student"?"Student code created.":"Staff email approved."); return; }
      const response = await fetch("/api/admin/invite-codes", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({role:inviteRole,level:inviteRole==="student"?inviteLevel:null,maxUses:inviteRole==="student"?inviteMaxUses:1,code:inviteRole==="student"?studentInviteCode:undefined,email:inviteRole!=="student"?staffInviteEmail:undefined}) });
      const result = await response.json();
      ...
      const displayValue=inviteRole==="student"?result.code:result.allowedEmail;
      setStaffCodes(current => [{id:result.id,code:displayValue,status:"Active",expires:inviteRole==="student"?`${inviteLevel} · ${inviteMaxUses} uses · expires in 48 hours`:`${inviteRole === "editor" ? "Curriculum editor" : "Demonstrator"} · approved email · expires in 48 hours`},...current]);
      if(inviteRole==="student")setStudentInviteCode("");else setStaffInviteEmail("");
      setToast(inviteRole === "student" ? "Student event code created. It expires in 48 hours." : `${inviteRole === "editor" ? "Curriculum editor" : "Demonstrator"} email approved for 48 hours.`);
    }
  ...
  function AnnouncementList({limit}:{limit?:number}) {
    return announcementRecords.length ? <div className="announcement-list">{announcementRecords.slice(0,limit).map(a => <article key={a.id??a.title}><div className="date-block"><strong>{a.date.split(" ")[0]}</strong><span>{a.date.split(" ")[1]}</span></div><div>{a.pinned && <span className="pin">PINNED</span>}<h3>{language === "hu" ? a.hu : a.title}</h3>{originDisclosure(a.contentOrigin,language)&&<span className="ai-label">{originDisclosure(a.contentOrigin,language)}</span>}<p>{a.text}</p></div></article>)}</div> : <div className="empty-state compact"><span>○</span><h3>No announcements yet</h3><p>New demonstrator messages will appear here.</p></div>;
  }
  ...
  function renderModal() {
    ...
    if (modal === "privacy") return <ModalShell title={language === "hu" ? "Adatvédelem és tartalmi átláthatóság · jóváhagyandó tervezet" : "Privacy and content transparency · draft for approval"} onClose={() => setModal(null)} wide><PrivacyNotice language={language} /></ModalShell>;
    if (modal === "notifications") return <ModalShell title="Notifications" onClose={() => setModal(null)}>{announcementRecords.length ? <div className="notification-list">{announcementRecords.slice(0,8).map(item => <article key={item.id??item.title} className={item.id&&readAnnouncementIds.includes(item.id)?"read":""}><span>●</span><div><strong>{item.title}</strong>{originDisclosure(item.contentOrigin,language)&&<span className="ai-label">{originDisclosure(item.contentOrigin,language)}</span>}<p>{item.text}</p></div></article>)}</div> : <div className="empty-state compact"><span>○</span><h3>You are all caught up</h3><p>Demonstrator announcements will appear in this inbox.</p></div>}<button className="secondary-button full-button" disabled={unreadAnnouncements.length===0} onClick={() => {void markAnnouncementsRead();completeAction("All notifications marked as read.");}}>Mark all as read</button></ModalShell>;
    if (modal === "lesson") {
      const live = curriculumRecords.find(item => item.week === selectedModule);
      const fallback = modules.find(item => item.week === selectedModule) || modules[3];
      const video = live?.assets.find(asset => asset.kind === "video" && asset.url);
      const images = live?.assets.filter(asset => asset.kind === "image" && asset.url) ?? [];
      const primaryMedia = video ?? images[0];
      const title = live ? (language === "hu" ? live.title_hu : live.title_en) : (language === "hu" ? fallback.hu : fallback.name);
      const equipment = live ? (language === "hu" ? live.equipment_hu : live.equipment_en) : "Use a needle holder, toothed forceps, scissors, 3-0 practice suture and a synthetic pad.";
      const technique = live ? (language === "hu" ? live.technique_hu : live.technique_en) : "Follow each step slowly and maintain consistent tissue handling.";
      const steps = live ? (language === "hu" ? live.steps_hu : live.steps_en) : ["Enter the tissue at a 90° angle.", "Follow the natural curve of the needle.", "Mirror the bite on the opposite side.", "Tie a secure square knot without excess tension."];
      return <ModalShell title={title} onClose={() => setModal(null)} wide><div className="lesson-layout"><div className="lesson-media">{video?.url ? <video src={video.url} controls preload="metadata"/> : images[0]?.url ? <Image src={images[0].url} width={720} height={480} unoptimized alt={images[0].caption??title}/> : <div className="lesson-video"><span>▶</span><small>TEACHING MEDIA</small></div>}{originDisclosure(primaryMedia?.content_origin,language)&&<span className="ai-label media-disclosure">{originDisclosure(primaryMedia?.content_origin,language)}</span>}{live?.video_url&&<a href={live.video_url} target="_blank" rel="noreferrer">Open external teaching video ↗</a>}{images.length>1&&<div className="lesson-image-strip">{images.slice(1).map(image=><figure key={image.id}><Image src={image.url!} width={180} height={120} unoptimized alt={image.caption??title}/>{originDisclosure(image.content_origin,language)&&<figcaption className="ai-label">{originDisclosure(image.content_origin,language)}</figcaption>}</figure>)}</div>}</div><div><span className="eyebrow">WEEK {String(live?.week??fallback.week).padStart(2,"0")} · {live?.level??fallback.level}</span>{originDisclosure(live?.content_origin,language)&&<span className="ai-label chapter-disclosure">{originDisclosure(live?.content_origin,language)}</span>}<h3>{language==="hu"?"Technikai útmutató":"Technique guide"}</h3>{live&&(language==="hu"?live.introduction_hu:live.introduction_en)&&<p>{language==="hu"?live.introduction_hu:live.introduction_en}</p>}<p>{technique}</p><h4>{language==="hu"?"Eszközök":"Equipment"}</h4><p>{equipment}</p><ol>{steps.map((step,index)=><li key={`${index}-${step}`}>{step}</li>)}</ol>{live&&(language==="hu"?live.application_hu:live.application_en)&&<><h4>{language==="hu"?"Alkalmazás":"Application"}</h4><p>{language==="hu"?live.application_hu:live.application_en}</p></>}<div className="editorial-record"><strong>{language==="hu"?"Emberi ellenőrzés":"Human review"}</strong><span>{live?.reviewer_name??(language==="hu"?"Ligatura adminisztrátor":"Ligatura administrator")}{live?.reviewed_at?` · ${new Date(live.reviewed_at).toLocaleDateString()}`:""}</span></div><div className="modal-actions"><button className="primary-button" onClick={() => setModal("upload")}>{t.submitWork} →</button></div></div></div></ModalShell>;
    }
    if (modal === "upload") return <ModalShell title={t.submitWork} onClose={() => {setModal(null);setSelectedFile("");selectedUpload.current=null;}}><form onSubmit={submitPhoto}><p className="modal-copy">Upload one clear JPG, PNG or WebP photo, up to 10 MB. It remains private between you and the demonstrator team.</p><button type="button" className={`dropzone ${selectedFile ? "has-file" : ""}`} onClick={() => fileInput.current?.click()}><span>{selectedFile ? "✓" : "↥"}</span><strong>{selectedFile || "Choose photo"}</strong></button><input ref={fileInput} type="file" accept="image/jpeg,image/png,image/webp" hidden onChange={e => {selectedUpload.current=e.target.files?.[0]??null;setSelectedFile(e.target.files?.[0]?.name || "");}} /><label className="reflection-label">Optional reflection<textarea name="reflection" maxLength={1000} placeholder="What felt easy or difficult?" /></label><label className="authenticity-confirmation"><input name="authenticityConfirmed" type="checkbox" required /><span><strong>Genuine-work confirmation</strong>This is an unaltered photograph of my own personal practice work. It is not AI-generated, materially manipulated, copied from another person or connected to a patient.</span></label><div className="modal-actions"><button className="secondary-button" type="button" onClick={() => setModal(null)}>{t.cancel}</button><button className="primary-button" type="submit" disabled={!selectedFile||dataBusy}>{dataBusy?"Uploading…":t.send}</button></div></form></ModalShell>;
    ...
    if (modal === "announcement") return <ModalShell title={t.newAnnouncement} onClose={() => setModal(null)}><form onSubmit={publishAnnouncement}><label className="form-field"><span>Title</span><input name="title" maxLength={160} required placeholder="Announcement title" /></label><label className="reflection-label">Message<textarea name="message" maxLength={4000} required placeholder="Write the announcement…" /></label><fieldset className="target-field"><legend>Target group</legend><div>{["Everyone","Beginner","Intermediate","Advanced"].map(x=><button type="button" className={announcementTarget===x?"active":""} onClick={()=>setAnnouncementTarget(x)} key={x}>{x}</button>)}</div></fieldset><label className="form-field"><span>Content origin</span><select value={announcementOrigin} onChange={event=>setAnnouncementOrigin(event.target.value as ContentOrigin)}><option value="human">Original human-written announcement</option><option value="ai_assisted">AI-assisted and fully human reviewed</option><option value="ai_generated">AI-generated or materially AI-altered</option></select></label><label className="check-row review-confirmation"><input name="editorialReviewConfirmed" type="checkbox" required /><span>I have reviewed this announcement and accept personal editorial responsibility for its accuracy.</span></label><label className="check-row"><input name="pinned" type="checkbox" /> Pin this announcement</label>{originDisclosure(announcementOrigin,language)&&<p className="publication-preview"><span className="ai-label">{originDisclosure(announcementOrigin,language)}</span>This label will appear next to the published announcement.</p>}<div className="modal-actions"><button className="secondary-button" type="button" onClick={() => setModal(null)}>{t.cancel}</button><button className="primary-button" type="submit">{t.publish}</button></div></form></ModalShell>;
...
function PrivacyNotice({ language }: { language: Language }) {
  return <div className="privacy-notice">
    <p className="privacy-warning"><strong>{language === "hu" ? "A valódi hallgatói regisztráció előtt jóváhagyás szükséges." : "Approval required before real student onboarding."}</strong> {language === "hu" ? "A Pécsi Sebészeti Társaságnak még meg kell erősítenie a hivatalos adatkezelő nevét, postai címét és adatvédelmi elérhetőségét." : "The official data-controller name, postal address and privacy contact must still be confirmed by the Surgical Society Pécs."}</p>
    <section className="ai-transparency-section"><span className="eyebrow">{language === "hu" ? "TARTALMI ÁTLÁTHATÓSÁG" : "CONTENT TRANSPARENCY"}</span><h3>{language === "hu" ? "Emberi felelősség és MI használat" : "Human responsibility and AI use"}</h3><p>{language === "hu" ? "A Ligatura jelenleg nem használ MI-t hallgatók értékelésére, jogosultsági döntésekre vagy automatikus orvosi oktatásra. A tananyagért, közleményekért és értékelésekért az azonosítható emberi szerző vagy ellenőrző felel. Az MI segítségével készült, MI által létrehozott vagy lényegesen módosított tartalmak közvetlenül a tartalom mellett egyértelmű jelölést kapnak." : "Ligatura does not currently use AI to assess students, decide eligibility or provide autonomous medical instruction. An identifiable human author or reviewer remains responsible for teaching material, announcements and assessments. AI-assisted, AI-generated or materially AI-altered content is labelled clearly beside the relevant content."}</p><p>{language === "hu" ? "A Ligatura nem használ érzelemfelismerést vagy biometrikus kategorizálást. Ha a jövőben MI-chatbot vagy MI-oktató kerül bevezetésre, a felhasználókat az első interakciónál egyértelműen tájékoztatjuk." : "Ligatura does not use emotion recognition or biometric categorisation. If an AI chatbot or AI tutor is introduced later, users will be informed clearly at the beginning of the interaction."}</p></section>
    ...
    <section><h3>Photographs</h3><p>Only genuine photographs of the student’s own personal practice work may be submitted for assessment. AI-generated, materially manipulated, copied or misleading submission images are prohibited. No patient, clinical image, identifying document or third-party personal information is permitted. Submission photographs remain private and are accessed through short-lived links.</p></section>
    <section><h3>Teaching media and recognisable people</h3><p>Curriculum media records whether it is human-created, AI-assisted, or AI-generated/materially altered. Media depicting a recognisable person may be uploaded only after consent is confirmed. Synthetic or altered representations are labelled next to the image or video and are never presented as documentary clinical footage.</p></section>
